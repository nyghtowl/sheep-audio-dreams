...
2. **Temporal embedded worker** — Each `app.py` runs a Temporal worker in a background thread. UI interactions trigger Temporal Updates (not signals), which block until completion and return results directly.
...
GitHub Actions runs pylint on push across Python 3.10, 3.11, 3.12.
